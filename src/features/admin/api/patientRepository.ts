  ...
  try {
    ...
    await db.transaction(async client => {
      ...
      await client.query(
        ...
        [newId, data.riskLevel, data.medicalNotes || null],
      );
    });
      ...
      if (data.riskLevel || data.status || data.medicalNotes) {
        await client.query(
          `UPDATE patient_profiles
           ...
           WHERE user_id = $4`,
          [data.riskLevel ?? null, data.status ?? null, data.medicalNotes ?? null, patientId],
        );
      }
    });
...
export async function sendPatientReminderAction(
  ...
  try {
    ...
    await db.query(`UPDATE patient_profiles SET last_reminder_sent = $1 WHERE user_id = $2`, [
      nowStr,
      patientId,
    ]);
    return { success: true };
