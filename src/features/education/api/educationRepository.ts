...
export async function getRelatedArticlesAction(
  ...
  try {
    const currentRes = await db.query<{ category: string }>(
...
export async function toggleArticleBookmarkAction(
  ...
  try {
    ...
    if (existing) {
      ...
      await db.query(`INSERT INTO user_bookmarks (user_id, article_id) VALUES ($1, $2)`, [
        userId,
        articleId,
      ]);
...
export async function isArticleBookmarkedAction(
  userId: string,
  articleId: string,
): Promise<boolean> {
  try {
